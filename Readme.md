...
> Use `--workers 1` because the in-memory `live_alerts` list and `_camera_states` dict are not shared across processes. For multi-process deployment, migrate these to Redis.
...
docker run -p 5000:5000 -v $(pwd)/videos:/app/videos traffic-iq
```

### Option C — Systemd service (Ubuntu)

```ini
[Unit]
Description=TrafficIQ Monitor
After=network.target

[Service]
User=ubuntu
WorkingDirectory=/opt/traffic-iq
ExecStart=/opt/traffic-iq/.venv/bin/gunicorn app:app --workers=1 --threads=8 --bind=0.0.0.0:5000
Restart=always
